...
- **Want to run the challenge?** Copy `evaluation/evaluation-prompt.txt` into any model, then open a Critique issue with the unedited output and the model version.
...
- **Translate it:** open a Translation issue first so nobody duplicates work, then a pull request adding `translations/<language-code>/`. Translations are Layer 2 and move fast, but a translation that changes meaning is a fork, so say where the language forced a choice.
...
- One open LIP per author at a time.
